  ...
  T: MemStroage,
{
    ...
    F: Fn(Field) -> bool,
  {
    ...
    if !self.is_in_mem(|| self.store.exist(&target.0)) && !self.is_in_history(closure, target.0) {
    ...
    F: Fn(Field) -> bool,
  {
    ...
    if self.is_in_mem(|| self.store.exist(&target.0)) {
      ...
      match mem_rec {
        Some(x) => match Self::state_transition(Self::get_option(x), RecInstruct::Update) {
